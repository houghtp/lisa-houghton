...
export const postsByCategoryQuery = groq`*[_type == "journalPost" && category == ] | order(publishedAt desc) {
  _id, title, slug, category, publishedAt, excerpt, coverImage, featured
}`;

export const postBySlugQuery = groq`*[_type == "journalPost" && slug.current == ][0] {
