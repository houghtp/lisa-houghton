...
export default async function CalendarPage() {
  ...
  const nextIdx = events.findIndex((e) => parseDate(e.startDate) >= today);
      ...
      <main className="flex-1 px-8 md:px-16 pt-16 md:pt-24 pb-20 md:pb-32">
        <div className="max-w-3xl mb-16 md:mb-20">
          ...
          <h1
            ...
            style={{ fontFamily: "var(--font-display)", fontWeight: 300, fontSize: "clamp(2.8rem, 7vw, 6.5rem)", lineHeight: 0.95, letterSpacing: "-0.01em", marginBottom: "1.5rem" }}
          >
            Catwalk<br />
          ...
          {events.map((event, i) => {
            ...
            return (
              <div
                ...
                style={{ borderTop: "1px solid var(--border)", display: "grid", gridTemplateColumns: "1fr auto", gap: "2rem", alignItems: "center", opacity: isPast ? 0.45 : 1 }}
              >
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    {isNext && (
                      <span className="text-xs tracking-widest uppercase" style={{ background: "var(--foreground)", color: "var(--background)", padding: "2px 8px", fontWeight: 500, letterSpacing: "0.1em", fontSize: "0.65rem" }}>
                        Next
                      </span>
                    )}
                    <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 400, fontSize: "clamp(1.1rem, 2.5vw, 1.4rem)", lineHeight: 1.2 }}>
                      {event.name}
                    </h2>
                  </div>
                  <p className="text-xs tracking-widest uppercase" style={{ color: "var(--muted)", fontWeight: 400 }}>
                    {event.city}
                  </p>
                  {event.note && (
                    <p className="mt-2 text-xs italic" style={{ color: "var(--muted)", fontWeight: 300 }}>
                      {event.note}
                    </p>
                  )}
                </div>
                <div style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                  <p style={{ fontFamily: "var(--font-display)", fontWeight: 300, fontSize: "0.95rem", color: isPast ? "var(--muted)" : "var(--foreground)" }}>
