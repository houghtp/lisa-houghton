      ...
      <main className="flex-1 px-8 md:px-16 pt-16 md:pt-24 pb-20 md:pb-32">
        <div className="max-w-2xl">
          ...
          <div className="grid md:grid-cols-3 gap-0 mb-12">
            {[
              { n: "01", heading: "First conversation", body: "We start with a free 20-minute call to understand where you are, what you want, and whether we&rsquo;re a good fit." },
              ...
              { n: "03", heading: "Practical support", body: "Between sessions I&rsquo;m available for quick questions, feedback on applications, and honest input when you need it." },
            ].map((step) => (
              <div key={step.n} className="py-8 pr-6 md:pr-10" style={{ borderTop: "1px solid var(--border)" }}>
                ...
                <p style={{ color: "var(--muted)", fontSize: "0.875rem", lineHeight: 1.75, fontWeight: 300 }} dangerouslySetInnerHTML={{ __html: step.body }} />
              </div>
            ))}
          </div>
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} style={{ maxWidth: "36rem" }}>
              ...
              <div>
                ...
                <textarea name="career" required placeholder="e.g. Final year at LCF, looking for my first role in womenswear..." rows={3} style={{ ...inputStyle, resize: "vertical" as const }} />
              </div>
              <div>
                <label style={labelStyle}>What are you hoping to get from mentoring?</label>
                <textarea name="goals" required placeholder="Tell me what you&apos;re working towards..." rows={4} style={{ ...inputStyle, resize: "vertical" as const }} />
              </div>
              <button
                type="submit"
                disabled={loading}
