...
function renderBlocks(blocks: any[]) {
  ...
  return blocks.map((block, i) => {
    ...
    if (style === "h2") {
      return (
        <h2 key={block._key || i} style={{ fontFamily: "var(--font-display)", fontWeight: 400, fontSize: "1.4rem", lineHeight: 1.2, marginTop: "2.5rem", marginBottom: "0.75rem" }}>
          {children}
        </h2>
      );
    }
    if (style === "blockquote") {
      return (
        <blockquote key={block._key || i} style={{ borderLeft: "2px solid var(--border)", paddingLeft: "1.5rem", marginLeft: 0, marginTop: "1.5rem", marginBottom: "1.5rem", color: "var(--muted)", fontStyle: "italic" }}>
          {children}
        </blockquote>
      );
    }
    return (
      <p key={block._key || i} style={{ marginTop: "1.25rem", lineHeight: 1.85, fontWeight: 300, fontSize: "1rem" }}>
        {children}
      </p>
    );
  });
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const issue = await client.fetch(newsletterIssueBySlugQuery, { slug: params.slug });
        ...
        <div className="max-w-2xl mx-auto w-full">
          ...
          <div className="mt-10 mb-12" style={{ borderBottom: "1px solid var(--border)", paddingBottom: "2rem" }}>
            <p className="text-xs tracking-widest uppercase mb-4" style={{ color: "var(--muted)", fontWeight: 400 }}>
              ...
              {issue.publishedAt && <span style={{ marginLeft: "1rem" }}>{formatDate(issue.publishedAt)}</span>}
            </p>
          ...
          {issue.lisaTake && issue.lisaTake.length > 0 && (
            <section className="mb-12">
              ...
              <div style={{ fontSize: "1.05rem" }}>{renderBlocks(issue.lisaTake)}</div>
            </section>
          )}
          {issue.handoffLine && (
            <p className="my-10 text-sm tracking-widest uppercase"
              style={{ color: "var(--muted)", fontWeight: 400, borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "1.5rem 0", letterSpacing: "0.08em" }}>
              {issue.handoffLine}
            </p>
          )}
          ...
          <div className="mt-16 pt-8 flex justify-between items-center" style={{ borderTop: "1px solid var(--border)" }}>
            <Link href="/newsletter" className="text-xs tracking-widest uppercase hover:opacity-60 transition-opacity"
              style={{ color: "var(--muted)", textDecoration: "none", fontWeight: 400 }}>
              ← All issues
            </Link>
